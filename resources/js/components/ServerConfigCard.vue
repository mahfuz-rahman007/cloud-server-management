...
const title = computed(() => {
  switch (props.type) {
    case 'cpu': return 'CPU Configuration'
    case 'ram': return 'Memory Configuration'
    case 'storage': return 'Storage Configuration'
    default: return ''
  }
})
    ...
    <CardContent class="space-y-3">
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        <button
          ...
          type="button"
          :class="cn(
            'relative flex flex-col items-start p-3 rounded-lg border-2 transition-all duration-200 text-left group hover:shadow-md',
        ...
        >
          ...
          <div
            :class="cn(
              'absolute top-1.5 right-1.5 w-3 h-3 rounded-full border-2 transition-all duration-200',
              selectedValue === option.value
          ...
          <div class="absolute top-1 left-1 flex gap-1">
            <Badge
              ...
              variant="default"
              size="sm"
              class="text-[10px] px-1 py-0.5 h-auto bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400"
            >
              ★
            </Badge>
            <Badge
              v-else-if="option.popular"
              variant="secondary"
              size="sm"
              class="text-[10px] px-1 py-0.5 h-auto"
            >
              ♦
            </Badge>
          </div>
          ...
          <div class="mt-4 w-full">
            <div class="font-semibold text-sm text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
              {{ option.label }}
            </div>
      ...
      <input
        :name="type === 'cpu' ? 'cpu_cores' : type === 'ram' ? 'ram_mb' : 'storage_gb'"
        :value="selectedValue"
        type="hidden"
      />
      ...
      <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
        <Label class="text-xs text-gray-500 dark:text-gray-400 mb-2">
        ...
        <div class="flex items-center gap-2">
          <input
            ...
            class="w-24 px-2 py-1 text-sm rounded border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
        ...
        <div class="text-xs dark:text-gray-500 mt-1">
          {{ type === 'cpu' ? 'Range: 1-128 cores' : type === 'ram' ? 'Range: 512MB - 1,048,576MB (1TB)' : 'Range: 10GB - 1,048,576GB (1PB)' }}
        </div>
      </div>
    </CardContent>
  </Card>
